...
library(tidyverse)
gs4_deauth()
d <- read_sheet("https://docs.google.com/spreadsheets/d/1YjPihpeGVk4cboGvEpKLOpcqZBf5P0tGClfsnKluuVs/edit?gid=0#gid=0")
```
...
d |>
  ...
  relocate(due_or_assign, .after = materials) |>
  gt() |>
  fmt_date(date, date_style = 31) |>
  sub_missing(columns = c(week, prepare, topic, materials, due_or_assign), missing_text = "") |>
  cols_align(
    align = "center", 
    columns = c(week)
  ) |>
  ...
  cols_align(
    align = "left", 
    columns = c(topic, prepare, materials, due_or_assign)
  ) |>
  tab_style(
    style = cell_borders(
      sides = "right",
